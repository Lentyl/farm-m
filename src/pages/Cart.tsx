...
import {addExtraProduct} from '../store/actions/loggedActions'
            ...
            <Container className='cart__container' fluid>
                <Form>
                    <ListGroup className='cart__list-container'>
                        {order && order.map((order, i) => {
                            return (
                                < ListGroup.Item key={i} className='cart__list-item'>
                                    ...
                                    <Button className="cart__item-add-btn" variant="light" onClick={() => {  dispatch(addExtraProduct('jabłka'))}}>+</Button>
                                    
                                    <Button className="cart__item-bin-btn" variant="light"><RiDeleteBin5Line className="cart__item-bin"/></Button>
                        ...
                        <ListGroup.Item className='cart__list-item'>{orderSum!==0?`Łączna kwota: ${orderSum} zł`:`Nie ddałeś nic do koszyka.`}</ListGroup.Item>
                    </ListGroup>
                </Form>
            </Container>
        </div>
